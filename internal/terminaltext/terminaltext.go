...
// Display escapes untrusted terminal output while retaining its line and tab
// structure. Command output is kept raw in the session/model transcript, but
// every terminal rendering boundary uses this form so CSI, OSC, carriage
// returns, and bidi controls cannot redraw a later permission prompt.
